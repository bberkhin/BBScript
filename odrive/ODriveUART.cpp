...
#include "../platform_util.h" // for miills()
...
template<class T> inline ODStream& operator << (ODStream &port, T value) { 
...
inline ODStream& operator<<(ODStream& port, const char* data) {
...
ODriveUART::ODriveUART(ODStream& serial) : serial_(serial) {}
...
ODriveFeedback ODriveUART::getFeedback() {
    ...
    if (spacePos >= 0) {
        return {
            (float)atof(response.substr(0, spacePos).c_str()),
            (float)atof(response.substr(spacePos+1).c_str())
        };
    } else {
        return {0.0f, 0.0f};
    }
...
void ODriveUART::setState(ODriveAxisState requested_state) {
    String strState = std::to_string((int)requested_state);
    setParameter(F("axis0.requested_state"), strState );
}
